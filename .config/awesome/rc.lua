...
beautiful.init("/home/user64/.config/awesome/themes/default/theme.lua")
...
tags = {}
for s = 1, screen.count() do
    -- Each screen has its own tag table. [Added screen 0]
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey, "Control"   }, "Right",
        function () 
		if awful.client.floating.get(client.focus) then
			awful.client.floating.set(client.focus, true) -- set idx
			w_diff = client.focus:geometry()['width']*.01
			h_diff = client.focus:geometry()['height']*.01
			awful.client.moveresize(-w_diff/2,-h_diff/2, w_diff, h_diff)
		else
    ...
    awful.key({ modkey, "Mod1"   }, "Up",
        function ()
		if awful.client.floating.get(client.focus) then
			awful.client.moveresize(0,-client.focus:geometry()['height']*.01,0,0)
		end
	end),
    ...
    awful.key({ modkey, "Mod1"   }, "Left",
        function ()
		if awful.client.floating.get(client.focus) then
			awful.client.moveresize(-client.focus:geometry()['width']*.01,0,0,0)
		end
	end),
...
-- Compute the maximum number of digit we need, limited to 9 [Changed to 10 for the 0 screen]
keynumber = 0
for s = 1, screen.count() do
...
-- Set wmname at wm startup
awful.util.spawn(setwmname)
